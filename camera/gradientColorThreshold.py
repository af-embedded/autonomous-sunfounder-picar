...
class GradientColorThreshold:
  ...
  def apply_hls(self, rgb_image):
    self.hls = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2HLS)
    self.l = self.hls[:, :, 1]
    self.s = self.hls[:, :, 2]
    self.z = np.zeros_like(self.s)
    return True
  def apply_l(self, gray_image):
    self.l = gray_image
    self.z = np.zeros_like(self.l)
    return True
  ...
  def apply_sobel_mask(self):
    ...
    slm, slx, sly = scale_abs(l_mag), scale_abs(lx), scale_abs(ly) # scale everything from 0 to 255
    b = self.z.copy()
